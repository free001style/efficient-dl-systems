...
class FSDPParam:
    ...
    def _init_sharded_param(self, param: nn.Parameter):
        ...
        sharded_data = distribute_tensor(
            tensor=param,
            device_mesh=self.mesh,
            placements=(self.fsdp_placement,),
        )
    ...
    def unsharded_param(self) -> nn.Parameter:  # ND
        if not hasattr(self, "_unsharded_param"):
            # Create unsharded param and save it to `self._unsharded_param`
            ...
            # We don't allocate storage yet, just create the tensor
            free_storage(unsharded_param)
...
class FSDPModule:
    ...
    def unshard(self):
        ...
        with record_function(self.with_fqn("FSDP::all_gather")):
            stream = self.comm_ctx.all_gather_stream
            with torch.cuda.stream(stream):
                for fsdp_param in self.fsdp_params:
                    ...
                    if isinstance(sharded_tensor, DTensor):
                        sharded_tensor = sharded_tensor.to_local()
            ...
            self._all_gather_event = stream.record_event()
    ...
    def _backward_prefetch(self) -> None:
        ...
        # Get the post-forward order to identify modules executed after this one
        # pass
        for post_forward_idx in self._post_forward_indices:
            next_idx = post_forward_idx - 1
            if next_idx >= 0:
                next_module = self.comm_ctx.post_forward_order[next_idx]
                if next_module.is_sharded:
...
def pre_forward(module: FSDPModule, args: Tuple[Any, ...], kwargs: Dict[str, Any]):
    ...
    with record_function(module.with_fqn("FSDP::pre_forward")):
        ...
        for i in range(len(module.comm_ctx.post_forward_order)):
            if i + 1 != len(module.comm_ctx.post_forward_order) and module.comm_ctx.post_forward_order[i] == module:
                module.comm_ctx.post_forward_order[i + 1].unshard()
        return args, kwargs
...
def post_backward(module: FSDPModule):
    ...
    with record_function(module.with_fqn("FSDP::post_backward_reduce")):
        ...
        stream = module.comm_ctx.reduce_scatter_stream
        with torch.cuda.stream(stream):
            for fsdp_param in module.fsdp_params:
                if not hasattr(fsdp_param, "_unsharded_param") or fsdp_param._unsharded_param.grad is None:
                    continue
                ...
                sharded_dtensor = DTensor.from_local(
                    sharded_grad,
                    fsdp_param.mesh,
                    placements=[fsdp_param.fsdp_placement],
                ) / fsdp_param.mesh.size(0)
                
                fsdp_param.sharded_param.grad = sharded_dtensor
